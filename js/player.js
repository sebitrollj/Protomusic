...
class ProtoMusicPlayer {
    ...
    initElements() {
        ...
        // Create Minimize Button for full player
        this.createMinimizeButton();
    }

    createMinimizeButton() {
        // Check if button already exists
        if (document.getElementById('minimizePlayerBtn')) return;

        // Find a good place to add the button (near close button if it exists)
        const fullPlayer = this.fullPlayerOverlay;
        if (!fullPlayer) return;

        // Create the button
        const minimizeBtn = document.createElement('button');
        minimizeBtn.id = 'minimizePlayerBtn';
        minimizeBtn.className = 'minimize-player-btn';
        minimizeBtn.title = 'Mini Player (Ctrl+M)';
        ...
        // Try to find header or controls area
        const header = fullPlayer.querySelector('.player-header, .full-player-header, .controls');
        if (header) {
            header.appendChild(minimizeBtn);
        } else {
            // Fallback: add to fullPlayer directly
            fullPlayer.insertBefore(minimizeBtn, fullPlayer.firstChild);
        }

        // Add CSS styles inline if stylesheet not loaded
        if (!document.getElementById('minimizeBtnStyles')) {
            const style = document.createElement('style');
            style.id = 'minimizeBtnStyles';
            style.textContent = `
                .minimize-player-btn {
                    position: absolute;
                    top: 20px;
        ...
        console.log('✓ Minimize button created');
    }
